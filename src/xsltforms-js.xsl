...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="xs math xforms" extension-element-prefixes="ixsl" version="3.0">
    ...
    <xsl:template name="xformsjs-main">
            ...
            <xsl:map>
                <xsl:for-each select="$xforms-doci/xforms:xform/xforms:model/xforms:bind">
                    ...
                    <xsl:if test="empty($xnodeset)">
                        <xsl:message>xformsbind is empty nodeste = <xsl:value-of
        ...
        <xsl:message> instance-jxml=<xsl:value-of
                select="serialize(xforms:convert-json-to-xml(ixsl:page()//script[@id = 'xforms-jinstance']/text()))"
            />
        </xsl:message>
    ...
    <xsl:function name="xforms:check-required-fields" as="item()*">
        ...
        <xsl:for-each select="$required-fieldsi">
            ...
            <xsl:variable name="resulti">
                <xsl:evaluate
                    xpath="concat('boolean(normalize-space(', @id, '))', '=', @id, '/', @data-required)"
            ...
            <xsl:otherwise>
                <xsl:variable name="error-message">
                    <xsl:for-each select="$required-fields-check">
                        ...
                        <xsl:value-of
                            select="concat('Value error see: ', serialize($curNode/@id), '&#10;')"/>
    ...
    <xsl:template match="xforms:input">
        ...
        <xsl:message>xforms:input=<xsl:value-of select="serialize($instance1)"/> ref= <xsl:value-of
                    ...
                    <xsl:for-each select="$selectedRepeatVar/*">
                        ...
                        <xsl:apply-templates select="$context/*">
                            ...
                            <xsl:with-param name="nodeset"
                                select="concat($nodeseti, '[', position(), ']', '/')"/>
                            <xsl:with-param name="bindings" select="$bindings"/>
                            <xsl:with-param name="submissions" select="$submissions"/>
                        </xsl:apply-templates>
                        </div>
    ...
    <xsl:template match="xforms:trigger">
        ...
        <xsl:variable name="actions">
            <action>
                ...
                <xsl:for-each
                    select="xforms:setvalue | xforms:insert | xforms:delete | xforms:action | xforms:toggle | xforms:send | xforms:setfocus | xforms:setindex | xforms:load | xforms:message | xforms:dispatch | xforms:rebuild | xforms:reset | xforms:show | xforms:hide | xforms:script | xforms:unload">
                    <xsl:apply-templates select="." mode="xforms-action"/>
                </xsl:for-each>
            </action>
        </xsl:variable>
        <xsl:message> actions (<xsl:value-of select="$myid"/>) = <xsl:value-of
                select="serialize($actions)"/></xsl:message>
    ...
    <xsl:template name="xformsjs-fill">
        ...
        <xsl:choose>
            <xsl:when test="empty($instance-xml) and empty($xforms-doc)">
                <xsl:sequence
                    select="ixsl:call(ixsl:window(), 'alert', 
                    ['App error: must supply either $instance-xml or $xforms-doc to xformsjs-fill'])"
                />
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="instance-doc">
                    <xsl:choose>
                        <xsl:when test="empty($instance-xml)">
                            <xsl:copy-of
                                select="$xforms-doc/xforms:xform/xforms:model/xforms:instance/*"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:copy-of select="$instance-xml"/>
                        </xsl:otherwise>
                ...
                <xsl:if test="exists($instance-doc)">
                    ...
                    <xsl:result-document href="{$xforms-instance-id}" method="ixsl:replace-content">
                        <xsl:value-of
                            select="xml-to-json(xforms:convert-xml-to-jxml($wrapped-instance))"/>
                    </xsl:result-document>
                </xsl:if>

                <xsl:message> xformsjs-fill instance-jxml=<xsl:value-of
                        select="serialize(xforms:convert-json-to-xml(ixsl:page()//script[@id = 'xforms-jinstance']/text()))"
                    />
                </xsl:message>
    ...
    <xsl:template match="*" mode="form-fill">
        ...
        <xsl:variable name="updatedPath"
            select="
                if ($isRepeatPath or $position > 1) then
                    concat($curPath,local-name(), '[', $position, ']')
                else concat($curPath,local-name())"/>

        <xsl:message>form-fill processing node:<xsl:value-of select="local-name()"/></xsl:message>
        <xsl:message>form-fill updatedPath:<xsl:value-of select="$updatedPath"/></xsl:message>
        ...
        <xsl:choose>
            <xsl:when test="exists($associated-form-control)">
                <xsl:message>Found associated form control with id: <xsl:value-of
                        select="$updatedPath"/></xsl:message>
                <ixsl:set-property name="value"
                    select="normalize-space(string-join(data(text()), ''))"
                    object="$associated-form-control"/>
            </xsl:when>
            <xsl:otherwise>
            ...
            <xsl:choose>
                <xsl:when test="count(current-group()) > 1">
                    <xsl:message>
                        <xsl:value-of
                            select="
                                concat(count(current-group()),
                                ' elements named ', current-grouping-key())"
                        />
                    </xsl:message>
    ...
    <xsl:template match="*" mode="form-check">
        ...
        <xsl:message>form-check processing node: <xsl:value-of select="local-name()"/></xsl:message>
        <xsl:message>form-check updatedPath: <xsl:value-of select="$updatedPath"/></xsl:message>
