...
<!--                  <xsl:sequence select="js:replaceDocument(serialize($responseXML))" />-->
                  ...
                  <xsl:choose>
                      <xsl:when test="$replace = 'instance' and not($targetref)">
...
<!--        <xsl:message use-when="$debugMode">[xforms:item] comparing value '<xsl:value-of select="xforms:value/text()"/>' against selected value '<xsl:value-of select="$selectedValue"/>'</xsl:message>-->
    ...
    <xsl:template match="button[exists(@data-action)]" mode="ixsl:onclick">
        ...
        <xsl:variable name="action-map" as="map(*)" select="js:getAction(string(@data-action))"/>
...
<!--                        <xsl:sequence select="js:setInstance($instance-with-insert)"/>-->
    ...
    <xsl:template match="xforms:action | xforms:setvalue | xforms:insert | xforms:delete | xforms:toggle | xforms:send | xforms:setfocus | xforms:setindex | xforms:load | xforms:message | xforms:dispatch | xforms:rebuild | xforms:reset | xforms:show | xforms:hide | xforms:script | xforms:unload">
        <xsl:variable name="myid"
            select="
            if (exists(@id)) 
            then @id
...
<!--        <xsl:variable name="updatedInstance" as="element()" select="$updatedInstanceDoc/*"/>-->
    ...
    <xsl:template name="getDataRef">
        ...
        <xsl:choose>
            ...
            <xsl:when test="exists($this/@nodeset)">
                <xsl:value-of select="$this/@nodeset"/>
            </xsl:when>
            <xsl:when test="exists($this/@ref)">
...
<!--                <xsl:message use-when="$debugMode">[getReferencedInstanceField] this-instance = <xsl:value-of select="serialize($this-instance)"/></xsl:message>-->
    ...
    <xsl:template name="setAction">
        ...
        <xsl:map>
            ...
            <xsl:if test="exists($this/@*:event)">
                <xsl:map-entry key="'@event'" select="xs:string($this/@*:event)" />
            ...
            <xsl:if test="$ifExecuted">
                ...
                <xsl:if test="exists($setvalueVar)">
                    ...
                    <xsl:if test="count(array:flatten($setValues)) gt 0">
                        ...
                        <xsl:variable name="updatedInstanceXML3" as="element()">                                
    ...
    <xd:doc scope="component">
        ...
        <xd:param name="form-control">HTML form control a @data-submit attribute referencing a registyered submission.</xd:param>
    </xd:doc>
